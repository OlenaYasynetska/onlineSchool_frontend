    ...
    >
      ...
      <dl class="mt-4 flex-1 space-y-2.5 text-sm">
        ...
        <div class="flex items-center justify-between gap-3">
          <dt class="text-slate-500">Tel.</dt>
        ...
        <div class="flex flex-col gap-0.5 sm:flex-row sm:items-start sm:justify-between sm:gap-3">
          <dt class="shrink-0 text-slate-500">Subjects</dt>
          <dd class="text-slate-700 sm:text-right">
            @if (teacher.subjectTitles.length) {
              {{ teacher.subjectTitles.join(', ') }}
            } @else {
              <span class="text-slate-400">—</span>
            }
          </dd>
        ...
        <div class="flex flex-col gap-0.5 sm:flex-row sm:items-start sm:justify-between sm:gap-3">
          ...
          <dd class="text-slate-700 sm:text-right">
            @if (teacher.groupNames.length) {
              {{ teacher.groupNames.join(', ') }}
            } @else {
              <span class="text-slate-400">—</span>
            }
          </dd>
